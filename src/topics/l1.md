...
Observed that `A[i]` can only put constraints on `A[i] - D` and `A[i] + D`, so we can split `A` into multiple chains. Each chain is an arithmetic sequence. For each chain, we can solve the following problems using dp: Given `P[0..M]`, find the minium total sum if for any adjacent element we must pick at least one.
...
[ABC437F](https://atcoder.jp/contests/abc437/submissions/71905553)
